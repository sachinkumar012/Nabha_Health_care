...
chatSessionSchema.pre("save", function (next) {
  if (!this.sessionId) {
    this.sessionId = `chat_${Date.now()}_${Math.random()
      .toString(36)
      .substr(2, 8)}`;
  }
...
chatSessionSchema.statics.findByContext = function (
  contextType,
  relatedId = null
) {
...
chatSessionSchema.statics.findPendingAssignment = function (
  department = null,
  skills = []
) {
  const query = {
...
chatSessionSchema.methods.assignTo = function (
  userId,
  assignedBy,
  department = null
) {
...
chatSessionSchema.methods.resolveSession = function (
  resolvedBy,
  resolutionType,
  summary = ""
) {
...
chatSessionSchema.methods.getUnreadMessages = function (userId) {
  return this.messages.filter(
    (message) => !message.readBy.some((read) => read.userId.equals(userId))
  );
};
...
chatSessionSchema.methods.calculateResponseTime = function () {
  ...
  this.messages.forEach((message) => {
    if (message.sender.role === "patient") {
      ...
      lastUserMessage &&
      (message.sender.role === "doctor" || message.sender.role === "support")
    ) {
      const responseTime =
        (message.timestamp - lastUserMessage.timestamp) / 1000; // in seconds
