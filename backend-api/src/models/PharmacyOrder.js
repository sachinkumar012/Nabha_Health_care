...
pharmacyOrderSchema.pre('save', function(next) {
  if (!this.orderId) {
    this.orderId = `PH${Date.now()}${Math.random().toString(36).substr(2, 6).toUpperCase()}`;
  }
  
  // Calculate total if not provided
  if (!this.summary.total) {
    this.summary.total = this.summary.subtotal + this.summary.deliveryCharges + this.summary.tax - this.summary.discount;
  }
...
// Static methods
pharmacyOrderSchema.statics.findByCustomer = function(customerId, status = null, limit = 20) {
...
pharmacyOrderSchema.methods.canCancel = function() {
  const cancellableStatuses = ['pending', 'prescription_verification', 'confirmed'];
...
pharmacyOrderSchema.methods.updateStatus = function(newStatus, note = '', updatedBy = 'system') {
