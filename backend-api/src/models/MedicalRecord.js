...
const medicalRecordSchema = new mongoose.Schema(
  {
    ...
    presentIllness: {
      ...
      maxlength: [
        2000,
        "Present illness description cannot exceed 2000 characters",
      ],
    },
    pastHistory: {
...
medicalRecordSchema.statics.findByDateRange = function (
  startDate,
  endDate,
  patientId = null
) {
...
medicalRecordSchema.statics.searchRecords = function (
  searchTerm,
  patientId = null
) {
  const query = {
...
// Instance methods
medicalRecordSchema.methods.addAuditEntry = function (
  action,
  performedBy,
  details = "",
  ipAddress = "",
...
medicalRecordSchema.methods.shareWith = function (
  userId,
  role,
  permissions = [],
  expiresAt = null
...
medicalRecordSchema.methods.canAccess = function (userId, permission = "read") {
  ...
  const share = this.privacy.sharedWith.find(
    (share) =>
  ...
  // Check permission
  return (
    share.permissions.includes(permission) || share.permissions.includes("full")
  );
};

medicalRecordSchema.methods.createAmendment = function (
  newData,
  amendedBy,
  reason
) {
