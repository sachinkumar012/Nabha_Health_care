...
hospitalSchema.virtual("isOpenNow").get(function () {
  ...
  if (
    !todayHours ||
    (!todayHours.is24Hours && (!todayHours.open || !todayHours.close))
  ) {
    return false;
  }
...
hospitalSchema.statics.findNearby = function (
  ...
  if (specialty)
    query["specialties.name"] = { $regex: specialty, $options: "i" };
...
hospitalSchema.statics.searchHospitals = function (searchTerm, options = {}) {
  ...
  return this.find(query)
    .sort(
      searchTerm ? { score: { $meta: "textScore" } } : { "ratings.overall": -1 }
    )
    .limit(limit)
    .skip(skip);
};
...
hospitalSchema.statics.findBySpecialty = function (specialty, options = {}) {
  ...
  return this.find(query).sort({ "ratings.overall": -1 }).limit(limit);
};
...
hospitalSchema.methods.getAvailableDoctors = function (
  specialty = null,
  day = null
) {
  let doctors = [];
