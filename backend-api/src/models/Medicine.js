...
const medicineSchema = new mongoose.Schema({
  ...
  category: {
    ...
    enum: [
      ...
      'drops', 'inhaler', 'powder', 'gel', 'cream', 'lotion',
...
medicineSchema.virtual('expiringSoon').get(function() {
  ...
  return this.batches.some(batch => 
    batch.expiryDate <= thirtyDaysFromNow && batch.quantity > 0
  );
});
...
medicineSchema.statics.searchMedicines = function(query, options = {}) {
  ...
  if (typeof requiresPrescription === 'boolean') searchQuery.requiresPrescription = requiresPrescription;
